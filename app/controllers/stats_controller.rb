...
class StatsController < ApplicationController
  def index
    ...
    @arr_by_days = [
      players.select { |z| z.days >= games_percent },
      "Посещаемость менее #{Stat::K_ATTENDANCE}%", players.select { |z| z.days < games_percent }
    ]
  end
  ...
  def ordering
    params[:sort].in?(%w[elo kp days games win draw lose goals_count assists_count goals_day_count assists_day_count]) ? params[:sort] : 'goals_day_count'
  end
end
